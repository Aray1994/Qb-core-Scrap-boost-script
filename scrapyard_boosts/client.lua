...
local function removeContractBlip()
    if contractBlip and DoesBlipExist(contractBlip) then
        SetBlipRoute(contractBlip, false)
...
RegisterNetEvent('scrapyard:startContract', function(data)
    ...
    removeContractBlip()
    contractBlip = AddBlipForCoord(data.coords.x, data.coords.y, data.coords.z)
    ...
    SetBlipColour(contractBlip, Config.ContractBlip.color or 5)
    SetBlipRoute(contractBlip, true)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(Config.ContractBlip.label or "Boosting Vehicle")
